...
source("./InfDis_DataPrep.R")
libbi_file <- "./InfDis_Model.bi"
...
dic <- c("DIC",DIC(posterior), rep(0,6))
...
inf_min_data <- obs$Minobs %>%
    rename(numer = value,
           times = time) %>%
    mutate(denom = input$Mininput[,"value"],
           value = numer/denom,
           Transition = "Z_min",
           error_low = as.double(unlist(binom.confint(numer,denom,conf.level = 0.95, method = "wilson")["lower"])),
           error_upp = as.double(unlist(binom.confint(numer,denom,conf.level = 0.95, method = "wilson")["upper"])))

inf_sub_data <- obs$Subobs %>%
    rename(numer = value,
           times = time) %>%
    mutate(denom = input$Subinput[,"value"],
           value = numer/denom,
           Transition = "Z_sub",
           error_low = as.double(unlist(binom.confint(numer,denom,conf.level = 0.95, method = "wilson")["lower"])),
           error_upp = as.double(unlist(binom.confint(numer,denom,conf.level = 0.95, method = "wilson")["upper"])))

zmin <- alt_fit_plot("Z_min",post_out,inf_min_data,"Infection to minimal") + coord_cartesian(ylim = c(0,0.3))
zsub <- alt_fit_plot("Z_sub",post_out,inf_sub_data,"Infection to subclinical") + ylim(c(0,0.1))
...
hist_plot
dev.off()
png(filename = "./Output/inf-min-plot.png",
    width = 290,
    height = 180,
    units = "mm",
    res = 600)
zmin
dev.off()
png(filename = "./Output/inf-sub-plot.png",
    width = 290,
    height = 180,
    units = "mm",
    res = 600)
...
plot_A <- (zmin + zsub) / fit_plots + plot_layout(heights = c(1,2))
plot_B <- zmin / zsub / fit_plots + plot_layout(design = "A###
                                                          B###
                                                          CCCC
                                                          CCCC
                                                          CCCC
...
png(filename = "./Output/all-plot_1.png",
    width = 290,
    height = 250,
    units = "mm",
    res = 600)
...
png(filename = "./Output/all-plot_2.png",
    width = 290,
    height = 250,
    units = "mm",
    res = 600)
