model TB_model {
    ...
    sub transition (delta = 0.1) {
	    ...
	    dRec/dt  =   minrec*Min1 + minrec*Min2 + minrec*Min3;
            ...
            dSub1/dt =   min_sub*Min1 + min_sub*Min2 - sub_min*Sub1 - sub_clin*Sub1;
        ...
        min_over_infect_p <- (
        ((sub_min + (infmin) * (sub_clin + sub_min)) * (clin_sub + mort)) - (infmin * clin_sub * sub_clin)
        ) / (
        ((min_sub + minrec) + (infmin) * min_sub) * (clin_sub + mort + sub_clin)
        )
